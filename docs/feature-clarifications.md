...
- [三十七、文件夹 + 标准文件库（Q246–Q252）](#三十七文件夹--标准文件库q246q252)
...
## 三十七、文件夹 + 标准文件库（Q246–Q252）

> 来源：文件夹设置审视——程序文件夹自定义增删改查（多级）+「标准文件库」呈现 + 编码前缀 / 路径 + 新建程序选文件夹。补 §33（Q230–Q233）未覆盖的模型决策，并修订 Q231。
> 编号：续 §36（Q242–Q245），本批 Q246–Q252。

### 37.1 Q246 「标准文件库」=用户文件夹树前端统称

**决策**：「标准文件库」是**所有非系统文件夹**（`tb_folder.system=false`）组成的用户文件夹树的**前端统称**，可建多级、可（在叶子）存程序。**不引入新表 / 新结构**——就是现有 `tb_folder` 树的前端呈现名称，与「废止」等 `system=true` 文件夹区分。

- 前端「标准文件库」导航区列出 `system=false` 的文件夹树。
...
| 项 | 落地 |
|----|------|
| Q246 | [editor-behavior.md](editor-behavior.md) 前端「标准文件库」导航；data-model §3.1 注明统称 |
...
左侧常驻「标准文件库」文件夹树（`system=false`，复用 Q246 + A12 `procedure_count`）；点叶子文件夹 → 右侧列出该文件夹程序，顶部面包屑显示 `full_path`。
...
「废止」（`system=true`）不在标准文件库树内，作为**树外独立入口 / 特殊节点**；点它列出已废弃 group，沿用 §22.16 deprecated 行特殊 UI。与 Q246（标准库=`system=false`）分离一致。

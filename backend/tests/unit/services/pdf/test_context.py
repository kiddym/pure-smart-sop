...
def test_chapter_tree_with_content_step_and_steps(db: Session, factory: Factory) -> None:
    ...
    factory.node(
        proc.id, body="<p>本程序用于…</p>", heading_level=None, kind="node", sort_order=2000
    )
...
def test_titles_derived_from_node_body(db: Session, factory: Factory) -> None:
    ...
    factory.node(
        proc.id,
        body="<p>检查阀门</p><p>步骤正文</p>",
        heading_level=None,
        kind="step",
        sort_order=2000,
    )
    factory.node(
        proc.id, body="<p>内容块正文</p>", heading_level=None, kind="node", sort_order=3000
    )
    node_numbering.recompute(db, proc.id)  # PDF reader 读持久化 node.code
    db.commit()
...
def test_nested_chapters_and_skip_numbering_from_nodes(db: Session, factory: Factory) -> None:
    ...
    factory.node(
        proc.id, body="<p>归口</p><p>x</p>", heading_level=None, kind="step", sort_order=3000
    )
    factory.node(
        proc.id,
        body="<p>附录</p>",

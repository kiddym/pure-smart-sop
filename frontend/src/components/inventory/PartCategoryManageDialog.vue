...
function openEdit(row: PartCategoryRead) {
  formMode.value = 'edit'
...
async function submitForm() {
  ...
  try {
    if (formMode.value === 'create') {
      await createPartCategory({ name: form.name.trim(), description: form.description })
    } else {
...
// ── delete ─────────────────────────────────────────────────
async function handleDelete(row: PartCategoryRead) {
  try {
...
<template>
  ...
  >
    <div class="toolbar">
      <el-button
        v-if="auth.hasPermission('part_category.manage')"
        type="primary"
        @click="openCreate"
      >
        新增分类
